...
def store_result(name, wpm, grade, type_, difficulty, new_score):
    ...
    # Find the correct position to insert the new score
    scores_list.sort(key=lambda x: x[1]['typeinc-mini score'], reverse=True)
    position = len(scores_list) - bisect.bisect_left([score[1]['typeinc-mini score'] for score in scores_list][::-1], new_score)
...
"""
    ...
    score_length = max(len(str(result['typeinc-mini score'])) for result in scores[difficulty].values())
    score_length = max(score_length, len("Score"))
    serial_length = 4

    stdscr.addstr(12, start_x, "{}Top 5 Highscore for typeinc-mini Test(local)\n".format(' '*(start_x-15)), curses.color_pair(6) | curses.A_BOLD)
    ...
    try:
        for index, time in enumerate(scores[difficulty], start=1):
            ...
            if top10 >= 5:
                break
            stdscr.addstr(i, start_x-3, f" | {index:<{serial_length}} | {result['name']:<{name_length}} | {result['wpm']:<{wpm_length}} | {' '.join(result['grade']):<{grade_length}} | {result['typeinc-mini score']:<{score_length}} |\n", curses.color_pair(1) | curses.A_BOLD)
